...
function add_product(){
    ...
    if(is_on_array === -1){
        ...
        if (products.length){
            ...
            alertify.alert('Error','No existe un producto con el código indeicado.')
        }
    }//if
...
function add_loaded_to_table(){
    ...
    $.each(last_bill_detail, function (i) {
        ...
        $.get(`/api/bill_detail/${last_bill_detail[i]}/`, function (data) {
           ...
           add_new_row(data.bill_detail_product_code, data.bill_detail_description, parseFloat(data.bill_detail_amount),
                        data.bill_detail_unit, parseFloat(data.bill_detail_price), parseFloat(data.bill_detail_total),
                        data.bill_detail_product, data.bill_detail_discount, data.bill_detail_iv)
...
function check_data_filled(){
    ...
    if(!bill.val()){
        bool = false;
        alertify.alert('Error en Factura del Proveedor','Debe Ingresar el numero de Factura del Proveedor');
        return bool
    }
    else{
        bool = true;
